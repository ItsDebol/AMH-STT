...
import numpy as np
import json
from scripts.Utility import spectrogram_from_file
from scripts.Utility import text_to_int_sequence
from python_speech_features import mfcc
import random
import librosa
from scripts.Utility import calc_feat_dim
import os
...
class AudioGenerator():
    ...
    def shuffle_data_by_partition(self, partition):
        ...
        """
        if partition == 'train':
            self.train_audio_paths, self.train_texts = shuffle_data(
                self.train_audio_paths, self.train_texts)
            self.train_length = len(self.train_texts)
        elif partition == 'valid':
            self.valid_audio_paths, self.valid_texts = shuffle_data(
    ...
    def load_metadata_from_desc_file(self, desc_file, partition):
        ...
        with open(desc_file, encoding='utf-8') as json_line_file:
            data = json.loads(json_line_file.read())
            for key ,value in data.items():
                if partition != "test":
                    audio_paths.append(os.path.join("../data/train/wav",key+".wav"))
                    texts.append(value)
